...
public class CellOLD {
    ...
    private List<CellOLD> adjacents;

    public CellOLD(int y, int x, boolean isMine) {
        this.y = y;
        this.x = x;
        this.isMine = isMine;
        isRevealed = false;
    ...
    public void computeAdjacents() {
        ...
        for (int i = 0; i < adjacentPoints.length - 1; i += 2) {
            ...
            if (board.isValidCoordinate(adjacentY, adjacentX)) {
                CellOLD cell = board.getCellAt(adjacentY, adjacentX);
                if (cell.isMine())
                    adjacentMineCount++;
                if (!cell.isRevealed()) {
    ...
    public void reveal() {
        ...
        if (this.adjacentMineCount == 0) {
            for (CellOLD adjacent : this.adjacents) {
